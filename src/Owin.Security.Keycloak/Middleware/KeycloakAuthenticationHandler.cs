...
namespace Owin.Security.Keycloak.Middleware
{
    internal class KeycloakAuthenticationHandler : AuthenticationHandler<KeycloakAuthenticationOptions>
    {
        ...
        public override async Task<bool> InvokeAsync()
        {
            ...
            if (!Options.ForceBearerTokenAuth && Request.Uri.GetLeftPart(UriPartial.Path) == callbackUri.ToString())
            {
                ...
                try
                {
                    ...
                    var properties = stateData[Constants.CacheTypes.AuthenticationProperties] as AuthenticationProperties ?? new AuthenticationProperties();
        ...
        private void SignInAsAuthentication(ClaimsIdentity identity, AuthenticationProperties authProperties = null, string signInAuthType = null)
        {
            ...
            var signInIdentity = signInAuthType != null ? new ClaimsIdentity(identity.Claims, signInAuthType, identity.NameClaimType, identity.RoleClaimType) : identity;
        ...
        private async Task LogoutRedirectAsync()
        {
            ...
            Response.Redirect((await KeycloakIdentity.GenerateLogoutUriAsync(Options, Request.Uri)).ToString());
        }
